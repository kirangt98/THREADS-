 ...
 *
 */
int SleepSeconds(int seconds)
{
    system_call_arguments_t sa;

    CHECKMODE;
    sa.call_id = SYS_SLEEP;
    sa.arguments[0] = (intptr_t)seconds;
    system_call(&sa);
    return (int)sa.arguments[3];
} /* end of Sleep */
...
int DiskRead(char *deviceName, void* dataBuffer, int platter, int track, int firstSector, int sectors, int* status)
{
    ...
    CHECKMODE;
    sa.call_id = SYS_DISKREAD;
    sa.arguments[0] = (intptr_t)deviceName;
    sa.arguments[1] = (intptr_t)dataBuffer;
    sa.arguments[2] = (intptr_t)platter;
    sa.arguments[3] = (intptr_t)track;
    sa.arguments[4] = (intptr_t)firstSector;
    sa.arguments[5] = (intptr_t)sectors;
    system_call(&sa);
 ...
 *  Arguments:
 *        char  *deviceName  -- which disk
 *        void  *dataBuffer  -- Buffer to with data to write
 *        int   platter      -- platter to write to
 *        int   track        -- first track to write to
 *        int   firstSector  -- first sector to write to
 *        int   sectors      -- number of sectors to write to
 *        int   *status      -- pointer to output value
 *                (output value: completion status)
 *
 *  Return Value: 0 means success, -1 means error occurs
...
int DiskInfo(char* deviceName, int* sectorSize, int *sectorCount, int* trackCount, int* platterCount)
{
    ...
    *sectorCount = (int)sa.arguments[1];
    *trackCount = (int)sa.arguments[2];
